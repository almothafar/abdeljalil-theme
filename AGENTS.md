...
[`.github/PULL_REQUEST_TEMPLATE.md`](.github/PULL_REQUEST_TEMPLATE.md) is what GitHub prefills a description with. Write prose under its headings, not a form: it asks only for what the diff cannot show — why this approach and not the one you rejected, what to watch for when the files are copied to the live site, and what you checked and did not check. Never restate the diff. Drop a heading you have nothing to say under, and add one where the change calls for it.
